...
const copy = {
  es: {
    ...
    languageEyebrow: 'Preferencias', languageTitle: 'Idioma de la aplicación', welcome: 'Bienvenida a casa', loginTitle: 'Tu café.<br>Tus sellos.', authLabel: 'Autenticación', phone: 'Teléfono', namePlaceholder: '¿Cómo te llamas?', emailPlaceholder: 'tu@email.com', privacy: 'Acepto la política de privacidad y el tratamiento de mis datos según el RGPD.', captchaRequired: 'Completa la comprobación de seguridad.', createAccount: 'Crear mi cuenta', signIn: 'Iniciar sesión', password: 'Contraseña', forgotPassword: 'He olvidado mi contraseña', sendRecovery: 'Enviar enlace de recuperación', backToSignIn: 'Volver al acceso', repeatPassword: 'Confirmar nueva contraseña', checkSession: 'Comprobando tu sesión…', authConfirmation: 'Revisa tu correo para confirmar la cuenta antes de iniciar sesión.', recoverySent: 'Si existe una cuenta con ese correo, recibirás un enlace de recuperación.', recoveryEyebrow: 'Seguridad de tu cuenta', recoveryTitle: 'Crea una nueva<br>contraseña.', recoveryCopy: 'Introduce una contraseña segura y repítela para confirmar que está escrita correctamente.', recoveryChecking: 'Validando el enlace de recuperación…', recoveryInvalidTitle: 'El enlace ya no es válido.', recoveryInvalidCopy: 'El enlace ha caducado, ya se ha utilizado o no puede verificarse. Solicita uno nuevo para continuar.', requestAnotherRecovery: 'Solicitar otro enlace', recoveryCompleteTitle: 'Contraseña actualizada.', recoveryCompleteCopy: 'Tu nueva contraseña ya está activa. Puedes continuar con tu cuenta Spirit.', recoveryCompleteSignIn: 'Contraseña actualizada correctamente. Inicia sesión con tu nueva contraseña.', continueToSpirit: 'Continuar en Spirit', recoverySessionMissing: 'No se ha podido validar el enlace. Solicita uno nuevo.', completeRecovery: 'Guardar nueva contraseña', emailConfirmationEyebrow: 'Activa tu cuenta', emailConfirmationProcessingTitle: 'Confirmando tu correo…', emailConfirmationProcessingCopy: 'Estamos confirmando tu correo…', emailConfirmationTitle: 'Correo confirmado', emailConfirmationCopy: 'Tu cuenta de Spirit Coffee ya está activada.', emailConfirmationSecondary: 'Ya puedes iniciar sesión con tu correo y contraseña.', emailConfirmationInvalidTitle: 'El enlace ya no es válido', emailConfirmationInvalidCopy: 'No hemos podido confirmar tu correo. El enlace puede haber caducado o ya haber sido utilizado.', emailConfirmationNetworkTitle: 'No se ha podido confirmar', emailConfirmationNetworkCopy: 'No hemos podido completar la confirmación en este momento. Inténtalo de nuevo.', returnHome: 'Volver al inicio',
...
function login() {
  ...
  const captcha = captchaConfiguration.configured ? '<div class="captcha-slot" data-captcha></div>' : '';
  const recoveryMode = ['recoveryChecking', 'recovery', 'recoveryError', 'recoverySuccess'].includes(state.authMode);
  let form;
  if (state.authMode === 'forgot') {
    form = `<form class="form" data-form="customer-forgot"><div class="field"><label for="forgot-email">${t('email')}</label><input id="forgot-email" name="email" type="email" autocomplete="email" inputmode="email" required></div>${captcha}${message}<button class="primary-button" type="submit" ${state.authLoading ? 'disabled' : ''}>${state.authLoading ? t('checkSession') : t('sendRecovery')}</button><button class="auth-link" type="button" data-action="auth-signin">${t('backToSignIn')}</button></form>`;
...
function emailConfirmation() {
  ...
  const actions = processing
    ? ''
    : `<button class="primary-button" type="button" data-action="email-confirmation-login">${t('signIn')}</button><button class="auth-link auth-link--centered" type="button" data-action="email-confirmation-home">${t('returnHome')}</button>`;
...
function bind() {
  ...
  document.querySelector('[data-form="customer-auth"]')?.addEventListener('submit',async(e)=>{e.preventDefault();if(state.authLoading)return;const form=e.currentTarget;const data=new FormData(form);const captchaToken=readCaptchaToken(form);if(captchaConfiguration.configured&&!captchaToken){state.authError=t('captchaRequired');render();return;}if(form.dataset.authMode==='signup'&&!passwordMeetsPolicy(data.get('password'))){state.authError=t('passwordLength');render();return;}state.authLoading=true;state.authError='';state.authNotice='';render();try{if(form.dataset.authMode==='signup'){const result=await signUpCustomer({email:data.get('email'),password:data.get('password'),displayName:data.get('name'),captchaToken,consent:{accepted:data.get('privacyConsent')==='accepted',version:PRIVACY_POLICY_VERSION}});if(result.confirmationRequired){state.authMode='signin';state.authNotice=t('authConfirmation');state.screen='login';}else{applyCustomerContext(result.context);await refreshCustomerLoyaltySafely();state.screen='home';}}else{applyCustomerContext(await signInCustomer(data.get('email'),data.get('password'),captchaToken));await refreshCustomerLoyaltySafely();state.screen='home';}}catch(error){state.authError=readableAuthError(error);state.screen='login';}finally{state.authLoading=false;render();}});
  document.querySelector('[data-form="customer-forgot"]')?.addEventListener('submit',async(e)=>{e.preventDefault();if(state.authLoading)return;const form=e.currentTarget;const data=new FormData(form);const captchaToken=readCaptchaToken(form);if(captchaConfiguration.configured&&!captchaToken){state.authError=t('captchaRequired');render();return;}state.authLoading=true;state.authError='';state.authNotice='';render();try{await requestCustomerPasswordReset(data.get('email'),captchaToken);state.authNotice=t('recoverySent');state.authMode='signin';}catch(error){state.authError=readableAuthError(error);}finally{state.authLoading=false;state.screen='login';render();}});
  ...
  document.querySelector('[data-form="password"]')?.addEventListener('submit',async(e)=>{e.preventDefault();const form=e.currentTarget;const data=new FormData(form);const current=data.get('currentPassword');const next=data.get('newPassword');const confirmation=data.get('confirmPassword');const captchaToken=readCaptchaToken(form);const error=form.querySelector('[data-password-error]');error.textContent='';if(!passwordMeetsPolicy(next)){error.textContent=t('passwordLength');return;}if(next!==confirmation){error.textContent=t('passwordMismatch');return;}if(captchaConfiguration.configured&&!captchaToken){error.textContent=t('captchaRequired');return;}try{await updateCustomerPassword(state.profile.email,current,next,captchaToken);document.querySelector('[data-sheet-backdrop]')?.remove();showToast(t('passwordSaved'));}catch(authError){error.textContent=readableAuthError(authError);}});
  document.querySelectorAll('[data-captcha]').forEach((element) => {
    mountCaptcha(element).catch((error) => {
      reportSecurityError('captcha-render', error);
      const formError = element.closest('form')?.querySelector('.form-error');
      if (formError) formError.textContent = t('operationError');
    });
  });
