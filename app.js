...
const topbar = () => `<header class="topbar"><div class="brand">${brandLogo()}</div><button class="icon-btn" data-toast="Próximamente: centro de avisos" aria-label="Notificaciones">${icons.menu}</button></header>`;
...
function rewards() {
  ...
  return `<main class="app-shell"><section class="screen">${topbar()}<p class="eyebrow">Tienes ${state.stamps} sellos</p><h1>Algo bueno<br>te espera.</h1><p class="subtitle">Canjea tus sellos en caja y disfruta de tu momento Spirit.</p><div class="section-head"><h2>Disponibles</h2></div><div class="cards">${rewards.map(([name,cost,emoji,n])=>`<article class="reward-card"><div class="reward-art">${emoji}</div><div><h3>${name}</h3><p>Preparado al momento con mucho mimo.</p><div class="reward-card__foot"><span class="cost">${cost}</span><button class="small-button" ${state.stamps<n?'disabled':''} data-redeem="${name}">${state.stamps<n?'Te faltan sellos':'Canjear'}</button></div></div></article>`).join('')}</div></section>${nav('rewards')}</main>`;
}
