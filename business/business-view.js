...
import {
  ...
  updateEmployeeSessionPreference
...
if (isBusinessRoute) {
  ...
  async function requireBusinessMfa(employee) {
    state.employee = employee;
    state.keepSession = setSessionPersistence(employee.keepSession !== false);
    state.error = '';
  ...
  function bind() {
    ...
    document.querySelector('[data-business-session-persistence]')?.addEventListener('change', async (event) => {
      const input = event.currentTarget;
      const previous = state.keepSession;
      input.disabled = true;
      try {
        const saved = await updateEmployeeSessionPreference(input.checked);
        state.keepSession = setSessionPersistence(saved);
        input.checked = state.keepSession;
      } catch (error) {
        state.keepSession = previous;
