...
import { versionedAvatarUrl } from '../services/avatar-service.js';
...
if (isBusinessRoute) {
  ...
  const employeeAvatar = () => {
    const source = versionedAvatarUrl(state.employee?.avatarUrl, state.employee?.avatarUpdatedAt);
    return source
      ? `<div class="employee-avatar" data-business-avatar><img src="${escapeHTML(source)}" alt="${escapeHTML(state.employee?.employeeName || 'Equipo Spirit')}"></div>`
      : `<div class="employee-avatar" data-business-avatar aria-hidden="true">${escapeHTML(employeeInitials())}</div>`;
  };
  ...
  function render() {
    ...
    app.querySelector('[data-business-avatar] img')?.addEventListener('error', (event) => {
      event.currentTarget.parentElement.textContent = employeeInitials();
    }, { once: true });
    bind();
  ...
  function bind() {
    ...
    document.querySelector('[data-business-session-persistence]')?.addEventListener('change', (event) => {
      state.keepSession = setSessionPersistence(event.currentTarget.checked);
      event.currentTarget.checked = state.keepSession;
    });
